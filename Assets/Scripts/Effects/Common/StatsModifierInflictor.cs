...
{
	[SerializeField] StatModInflictorDescriptor descriptor;

	[SerializeField] public StatModifierContext statModifierContext;
	...
	public override void StopEffect(IEffectContextHolder mono)
	{
		...
		Debug.Log("Restored " + statModContext.Stat + " " + statModContext.RestoreValue + " to " +
		          mono.BasicStats[(int)statModifierContext.Stat].value);
	}
	...
	protected override IEffectContextData AttachData(IEffectContextHolder mono, TimeEffectContext timeEffectContext)
	{
		...
		Debug.Log("Affected " + statModContext.Stat + " " + statModContext.RestoreValue + " to " +
		          mono.BasicStats[(int)statModifierContext.Stat].Value);
	...
	{
		[SerializeField] StatEnum stat;

		[SerializeField] [Range(-100f, 100f)] float percentDecrease;
		...
		public TimeEffectContext TimeEffect
		{
			get => timeEffect;
