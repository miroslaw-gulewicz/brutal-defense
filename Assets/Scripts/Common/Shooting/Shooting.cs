...
public class Shooting : MonoBehaviour
{
    ...
    [SerializeField]
    private IndicatorBarBehaviour _nextAttackTimerBar;
    ...
    public TargetingSystem TargetingSystem { get => _targetingSystem; set => _targetingSystem = value; }
    ...
    private void TriggerExit(Collider obj)
    {
        if (obj.TryGetComponent(out Agent dest))
            _targetingSystem.TargetExits(dest);
    }

    private void TriggerEnter(Collider obj)
    {
        if (obj.TryGetComponent(out Agent dest))
            _targetingSystem.TargetEnters(dest);
    }
